...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ content, onUpdate }) => {
  const [activeTab, setActiveTab] = useState<'site' | 'home' | 'courses' | 'notices' | 'gallery' | 'form' | 'contact' | 'footer'>('site');
  ...
  const addSupportLink = () => {
    setLocalContent(prev => ({
      ...
      site: {
        ...
        footer: {
          ...prev.site.footer,
          supportLinks: [...prev.site.footer.supportLinks, { label: 'New Link', path: '#' }]
        }
      }
    }));
  };
  ...
  const updateSupportLink = (index: number, field: 'label' | 'path', value: string) => {
    setLocalContent(prev => ({
      ...
      site: {
        ...
        footer: {
          ...prev.site.footer,
          supportLinks: prev.site.footer.supportLinks.map((l, i) => i === index ? { ...l, [field]: value } : l)
        }
      }
    }));
  };
  ...
  const deleteSupportLink = (index: number) => {
    setLocalContent(prev => ({
      ...
      site: {
        ...
        footer: {
          ...prev.site.footer,
          supportLinks: prev.site.footer.supportLinks.filter((_, i) => i !== index)
        }
      }
    }));
  };
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 pb-20">
      ...
      <div className="container mx-auto px-4 mt-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 space-y-2 shrink-0">
          {(['site', 'home', 'courses', 'notices', 'gallery', 'form', 'contact', 'footer'] as const).map(tab => (
            <button
              key={tab}
            ...
            >
              <i className={`fa-solid fa-${
                ...
                tab === 'footer' ? 'shoe-prints' : 'wpforms'
              } text-lg`}></i>
        ...
        <div className="flex-grow bg-slate-800 rounded-[2.5rem] p-8 md:p-12 border border-slate-700 shadow-3xl overflow-hidden min-h-[70vh]">
          ...
          {activeTab === 'footer' && (
            <div className="space-y-12 animate-fade-in">
              <SectionHeader title="Footer Architecture" />
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="space-y-8 bg-slate-900/30 p-8 rounded-[2.5rem] border border-slate-700">
                  <h3 className="text-emerald-500 font-black text-lg flex items-center gap-3">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <textarea value={localContent.site.footer.brandDescription} onChange={e => updateFooterField('brandDescription', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-200 font-medium resize-none" rows={3} />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Quick Links Title</label>
                        <input value={localContent.site.footer.quickLinksLabel} onChange={e => updateFooterField('quickLinksLabel', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-200 font-medium" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Support Column Title</label>
                        <input value={localContent.site.footer.supportLinksLabel} onChange={e => updateFooterField('supportLinksLabel', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-200 font-medium" />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Reach Us Column Title</label>
                        <input value={localContent.site.footer.reachUsLabel} onChange={e => updateFooterField('reachUsLabel', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-200 font-medium" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Bottom Text (Credits)</label>
                        <input value={localContent.site.footer.bottomText} onChange={e => updateFooterField('bottomText', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-200 font-medium" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8 bg-slate-900/30 p-8 rounded-[2.5rem] border border-slate-700">
                  ...
                  <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    {localContent.site.footer.supportLinks.map((link, idx) => (
                      <div key={idx} className="bg-slate-800 p-4 rounded-2xl border border-slate-700 space-y-2 group">
                        ...
                        <input 
                          value={link.path} 
                          onChange={e => updateSupportLink(idx, 'path', e.target.value)} 
                          className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-emerald-500 font-mono" 
