        ...
        <div className="space-y-6">
          {legal[page].sections.map((section, idx) => (
            <div key={section.id} className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700 relative group">
              <button onClick={() => deleteSection(page, section.id)} className="absolute -top-2 -right-2 w-8 h-8 bg-red-600 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center shadow-xl z-10"><i className="fa-solid fa-trash text-[10px]"></i></button>
              <div className="space-y-4">
