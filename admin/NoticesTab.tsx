    ...
    <div className="space-y-6">
      {notices.map(notice => (
        <div key={notice.id} className="bg-slate-900/50 p-6 rounded-[1.5rem] border border-slate-700 flex gap-4 hover:border-emerald-500/30 transition-all">
          ...
          <button onClick={() => deleteItem(notice.id)} className="text-red-500 self-center p-3 hover:bg-red-500/10 rounded-xl transition-colors"><i className="fa-solid fa-trash-can"></i></button>
        </div>
      ))}
    </div>
  </div>
